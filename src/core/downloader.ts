...
const checkChecksums = async (
  ...
  if (checksums?.sha256sum) {
    ...
    const sha256sum = spawnSync('sha256sum', [filePath], { encoding: 'utf-8' })
      .stdout.trim()
      .split(' ')[0];
  ...
  if (checksums?.md5sum) {
    ...
    const md5sum = spawnSync('md5sum', [filePath], { encoding: 'utf-8' })
      .stdout.trim()
      .split(' ')[0];
...
const downloadFileCurl = async (
  ...
  const status = spawnSync(CURL_PATH, args, { stdio: 'inherit' }).status;

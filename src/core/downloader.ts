...
const checkChecksums = async (filePath: string, checksums: Checksums): Promise<void> => {
    if (checksums?.sha256sum) {
        logger.verbose(`Checking SHA256 checksum of ${filePath}`);
        let sha256sum = spawnSync('sha256sum', [filePath], { encoding: 'utf-8' }).stdout.trim().split(' ')[0];
    ...
    if(checksums?.md5sum) {
        ...
        let md5sum = spawnSync('md5sum', [filePath], { encoding: 'utf-8' }).stdout.trim().split(' ')[0];
...
const checkIfSizeCorrect = async (filePath: string, download_size: number, checksums: Checksums = undefined): Promise<void> => {
    ...
    logger.warn(`Downloaded file ${filePath} is not the correct size. Checking checksums...`);
...
const downloadFileCurl = async (url: string, downloadPath: string): Promise<void> => {
...
const downloadFile = async (url: string, downloadPath: string, download_size: number = 0, checksums: Checksums = undefined ): Promise<void> => {
    fs.mkdirSync(TEMP_DL_DIR, { recursive: true });
    let tempFile = path.join(TEMP_DL_DIR, crypto.randomBytes(32).toString('hex'));
    const downloaderFunc = DOWNLOADER.toLocaleLowerCase() === 'axios' ? downloadFileAxios : downloadFileCurl;
    return downloaderFunc(url, tempFile).then(_ => {
